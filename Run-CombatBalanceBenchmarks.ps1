...
function Invoke-Strategy([string]$Scenario,[string]$Strategy,[int]$Seed){
 ...
 while($c.active-and$c.combatants.armand.alive-and$turns-lt30){
  ...
  $turns++;$cmd=switch($Strategy){'straight'{'Eldritch Blast the enemy.'}'darkness'{if(-not$usedSetup){$usedSetup=$true;'Cast Darkness.'}else{'Eldritch Blast the enemy.'}}'flee'{'Flee into the adjacent room and hide.'}'guillermo'{'Tell Guillermo to reposition nearby and Eldritch Blast the enemy.'}'creative'{if(-not$usedSetup){$usedSetup=$true;'Trip the enemy.'}else{'Eldritch Blast the enemy.'}}'affordance'{if(-not$usedSetup){$usedSetup=$true;'Bring down the oil lamp on the enemy.'}else{'Eldritch Blast the enemy.'}}}
...
$runs=@();foreach($scenario in 'A','B','C','D','E'){foreach($strategy in 'straight','darkness','flee','guillermo','creative','affordance'){foreach($seed in 1..20){$runs+=Invoke-Strategy $scenario $strategy $seed}}}
