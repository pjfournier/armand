...
function Invoke-IntentInterpretation {
 ...
 $backendResult=$null
 try{$backendResult=if($null-ne$Backend){&$Backend $PlayerText $Context $schema $Seed}else{Invoke-CommaIntentBackend $PlayerText $Context $schema -Seed $Seed};$raw=[string]$backendResult.RawOutput;$parseText=$raw-replace'\s*\[end of text\]\s*$','';$intent=$parseText|ConvertFrom-Json;foreach($field in @('secondary_target','purpose','modifiers')){if(-not$intent.PSObject.Properties[$field]){$intent|Add-Member -NotePropertyName $field -NotePropertyValue $(if($field-eq'modifiers'){@()}else{$null})}};if($null-ne$pronounTarget){$intent.target=$pronounTarget}}
