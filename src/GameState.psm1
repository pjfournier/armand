...
function Add-ItemState { param($State,[string]$Id,[string]$Location,[string]$Owner,[hashtable]$ItemState=@{},[string[]]$Tags=@(),[string]$DisplayName=$Id,[string[]]$Aliases=@(),[string[]]$InteractableParts=@(),[ValidateSet('static-renderable','animate-excluded')][string]$RenderClass='static-renderable') $State.Items[$Id]=[pscustomobject]@{Id=$Id;DisplayName=$DisplayName;Aliases=@($Aliases);Location=$Location;Owner=$Owner;State=[ordered]@{};Tags=@($Tags);InteractableParts=@($InteractableParts);RenderClass=$RenderClass};foreach($key in $ItemState.Keys){$State.Items[$Id].State[$key]=$ItemState[$key]} }
