...
function Add-LocationState { param($State,[string]$Id,[string[]]$VisibleObjects=@(),[string[]]$Exits=@()) $State.Locations[$Id]=[pscustomobject]@{Id=$Id;VisibleObjects=@($VisibleObjects);PersistentState=[ordered]@{};NPCsPresent=[Collections.Generic.List[string]]::new();Exits=@($Exits)} }
function Add-ItemState { param($State,[string]$Id,[string]$Location,[string]$Owner,[hashtable]$ItemState=@{},[string[]]$Tags=@()) $State.Items[$Id]=[pscustomobject]@{Id=$Id;Location=$Location;Owner=$Owner;State=[ordered]@{};Tags=@($Tags)};foreach($key in $ItemState.Keys){$State.Items[$Id].State[$key]=$ItemState[$key]} }
function Add-NpcState { param($State,[string]$Id,[string]$Location,[int]$Awareness=0,[int]$Suspicion=0,[string]$SearchState='idle',[string]$Goal='',[string[]]$KnowledgeReferences=@(),[string]$Target='') $State.NPCs[$Id]=[pscustomobject]@{Id=$Id;CurrentLocation=$Location;Awareness=$Awareness;Suspicion=$Suspicion;SearchState=$SearchState;Goal=$Goal;KnowledgeReferences=@($KnowledgeReferences);Target=$Target} }
...
Export-ModuleMember -Function New-GameState,Add-LocationState,Add-ItemState,Add-NpcState,Add-ObjectiveState,Set-ItemProperty,Move-Npc,Add-GameTime,Move-InventoryItem,Invoke-SpellSlotUse
