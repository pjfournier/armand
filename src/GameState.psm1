...
function Add-ItemState { param($State,[string]$Id,[string]$Location,[string]$Owner,[hashtable]$ItemState=@{},[string[]]$Tags=@(),[string]$DisplayName=$Id,[string[]]$Aliases=@(),[string[]]$InteractableParts=@()) $State.Items[$Id]=[pscustomobject]@{Id=$Id;DisplayName=$DisplayName;Aliases=@($Aliases);Location=$Location;Owner=$Owner;State=[ordered]@{};Tags=@($Tags);InteractableParts=@($InteractableParts)};foreach($key in $ItemState.Keys){$State.Items[$Id].State[$key]=$ItemState[$key]} }
