...
function ConvertTo-EngineIntent {param([Parameter(Mandatory)]$Intent)[pscustomobject]@{Actor=$(if($Intent.actor-eq'guillermo'){'Guillermo'}else{'Armand'});Action=$Intent.action;Target=$Intent.target;SecondaryTarget=$(if($Intent.PSObject.Properties['secondary_target']){$Intent.secondary_target}else{$null});Method=$Intent.method;Area=$Intent.area;CommunicativeIntent=$Intent.communicative_intent;Spell=$Intent.spell;Purpose=$(if($Intent.PSObject.Properties['purpose']){$Intent.purpose}else{$null});Modifiers=$(if($Intent.PSObject.Properties['modifiers']){@($Intent.modifiers)}else{@()});RawInput=$Intent.raw_input}}
